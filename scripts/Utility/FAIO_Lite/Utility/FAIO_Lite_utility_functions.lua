...
function FAIO_Lite_utility_functions.utilityRoundNumber(number, digits)
...
function FAIO_Lite_utility_functions.utilityGetTableLength(table)

	if not table then return 0 end
	if table == nil or next(table) == nil then return 0 end
...
function FAIO_Lite_utility_functions.utilityIsInTable(table, arg)

	if not table then return false end
	if not arg then return false end
	if table == nil or next(table) == nil then return false end
...
function FAIO_Lite_utility_functions.castLinearPrediction(myHero, enemy, adjustmentVariable, fastPos, castRange)

	if not myHero then return end
	if not enemy then return end

	local useFastestPos = fastPos
		if useFastestPos == nil then
			useFastestPos = false
		end
		if castRange == nil then
			useFastestPos = false
		else
			if castRange <= 50 then
				useFastestPos = false
			end
		end
	...
	if not useFastestPos then
		if enemyRotation and enemyOrigin then
			if not NPC.IsRunning(enemy) then
				return enemyOrigin
			else return enemyOrigin:__add(enemyRotation:Normalized():Scaled(FAIO_Lite_utility_functions.GetMoveSpeed(enemy) * (adjustmentVariable + FAIO_Lite_utility_functions.humanizerMouseDelayCalc(enemyOrigin) + FAIO_Lite_utility_functions.TimeToFacePosition(myHero, enemyOrigin)) * (1 - cosGamma)))
			end
		end
	else
		if enemyRotation and enemyOrigin then
			if not NPC.IsRunning(enemy) then
				local startVec = Entity.GetAbsOrigin(myHero) + (enemyOrigin - Entity.GetAbsOrigin(myHero)):Normalized():Scaled(50)
				local endVec = Entity.GetAbsOrigin(myHero) + (enemyOrigin - Entity.GetAbsOrigin(myHero)):Normalized():Scaled(castRange - 50)
				local fastPoint = Entity.GetAbsOrigin(myHero) + (FAIO_Lite_utility_functions.GetClosestPoint(startVec, endVec, Input.GetWorldCursorPos(), segmentClamp) - Entity.GetAbsOrigin(myHero))
				return fastPoint
			else
				local predPos = enemyOrigin:__add(enemyRotation:Normalized():Scaled(FAIO_Lite_utility_functions.GetMoveSpeed(enemy) * (adjustmentVariable + FAIO_Lite_utility_functions.humanizerMouseDelayCalc(enemyOrigin) + FAIO_Lite_utility_functions.TimeToFacePosition(myHero, enemyOrigin)) * (1 - cosGamma)))
				local startVec = Entity.GetAbsOrigin(myHero) + (predPos - Entity.GetAbsOrigin(myHero)):Normalized():Scaled(50)
				local endVec = Entity.GetAbsOrigin(myHero) + (predPos - Entity.GetAbsOrigin(myHero)):Normalized():Scaled(castRange - 50)
				local fastPoint = Entity.GetAbsOrigin(myHero) + (FAIO_Lite_utility_functions.GetClosestPoint(startVec, endVec, Input.GetWorldCursorPos(), segmentClamp) - Entity.GetAbsOrigin(myHero))
				return fastPoint
			end
		end
...
--[[
function FAIO_Lite_utility_functions.castLinearPrediction(myHero, enemy, adjustmentVariable)
...
--]]
function FAIO_Lite_utility_functions.castPrediction(myHero, enemy, adjustmentVariable)
	...
	if enemyRotation and enemyOrigin then
			if not NPC.IsRunning(enemy) then
				...
				return enemyOrigin:__add(enemyRotation:Normalized():Scaled(FAIO_Lite_utility_functions.GetMoveSpeed(enemy) * (adjustmentVariable + FAIO_Lite_utility_functions.humanizerMouseDelayCalc(enemyOrigin) + FAIO_Lite_utility_functions.TimeToFacePosition(myHero, enemyOrigin))))
			end
	end
end

function FAIO_Lite_utility_functions.isEnemyTurning(enemy)
	...
	if NPC.IsRunning(enemy) then
		table.insert(FAIO_Lite_utility_functions.rotationTable, rotationSpeed)
			if #FAIO_Lite_utility_functions.rotationTable > (Menu.GetValue(FAIO_Lite_options.optionKillStealInvokerTurn) + 1) then
				table.remove(FAIO_Lite_utility_functions.rotationTable, 1)
			end
	end
	
	if #FAIO_Lite_utility_functions.rotationTable < Menu.GetValue(FAIO_Lite_options.optionKillStealInvokerTurn) then 
		return true
	else
		local rotationSpeedCounter = 0
		i = 1
		repeat
			rotationSpeedCounter = rotationSpeedCounter + FAIO_Lite_utility_functions.rotationTable[#FAIO_Lite_utility_functions.rotationTable + 1 - i]
			i = i + 1
		until i > Menu.GetValue(FAIO_Lite_options.optionKillStealInvokerTurn)

		if rotationSpeedCounter / Menu.GetValue(FAIO_Lite_options.optionKillStealInvokerTurn) <= 10 then
			return false
		else
			return true
		end
...
function FAIO_Lite_utility_functions.GetMoveSpeed(enemy)
...
function FAIO_Lite_utility_functions.getBestPosition(unitsAround, radius)
	...
	return FAIO_Lite_utility_functions.getMidPoint(unitsAround)



end

function FAIO_Lite_utility_functions.getMidPoint(entityList)
...
function FAIO_Lite_utility_functions.GetMyFaction(myHero)
...
function FAIO_Lite_utility_functions.GetMyFountainPos(myHero)

	if not myHero then return end

	local myFaction 
		if myFaction == nil then 
			myFaction = FAIO_Lite_utility_functions.GetMyFaction(myHero)
		end
...
function FAIO_Lite_utility_functions.GetEnemyFountainPos(myHero)
...
function FAIO_Lite_utility_functions.IsCreepAncient(npc)
...
function FAIO_Lite_utility_functions.inSkillAnimation(myHero)

	if not myHero then return false end

	local abilities = {}

	if abilities == nil or next(abilities) == nil then
		for i = 0, 25 do
...
function FAIO_Lite_utility_functions.TimeToFacePosition(myHero, pos)
	...
	local turnTime = FAIO_Lite_utility_functions.utilityRoundNumber(((0.033 * math.pi / myTurnRate) / 180) * checkAngle, 3)

	return turnTime

end

function FAIO_Lite_utility_functions.GetLongestCooldown(myHero, skill1, skill2, skill3, skill4, skill5)
...
function FAIO_Lite_utility_functions.TargetDisableTimer(myHero, enemy)
	...
	if searchMod then
		if NPC.HasModifier(enemy, Modifier.GetName(searchMod)) then
			...
			elseif Modifier.GetName(searchMod) == "modifier_faceless_void_chronosphere_freeze" then
				return Modifier.GetCreationTime(searchMod) + (3.5 + FAIO_Lite_utility_functions.GetTeammateAbilityLevel(myHero, "faceless_void_chronosphere") * 0.5)
			else
...
function FAIO_Lite_utility_functions.TargetIsHexed(myHero, npc)
...
function FAIO_Lite_utility_functions.HexTimeLeft(myHero, npc)
...
function FAIO_Lite_utility_functions.GetTeammateAbilityLevel(myHero, ability)
...
function FAIO_Lite_utility_functions.TargetIsInvulnarable(myHero, enemy)
...
function FAIO_Lite_utility_functions.GetClosestPoint(A,  B,  P, segmentClamp)
...
function FAIO_Lite_utility_functions.IsNPCinDanger(myHero, npc)
	...
	if FAIO_Lite_utility_functions.IsHeroInvisible(npc) == true then return false end
	...
	if (Entity.GetAbsOrigin(myHero) - FAIO_Lite_utility_functions.GetMyFountainPos(myHero)):Length2D() < 1500 then return end
	...
	if FAIO_Lite_utility_functions.TargetGotDisableModifier(myHero, npc) == true or (NPC.IsSilenced(npc) and not momSilenced) or
		NPC.HasModifier(npc, "modifier_item_nullifier_mute") or NPC.HasState(npc, Enum.ModifierState.MODIFIER_STATE_HEXED) then

		if Entity.GetHealth(npc) / Entity.GetMaxHealth(npc) <= (Menu.GetValue(FAIO_Lite_options.optionDefensiveItemsThresholdDisable) / 100) then
	...
	if Entity.GetHealth(npc) <= Menu.GetValue(FAIO_Lite_options.optionDefensiveItemsThreshold)/100 * Entity.GetMaxHealth(npc) then
...
function FAIO_Lite_utility_functions.IsHeroInvisible(myHero)
...
function FAIO_Lite_utility_functions.TargetIsStunnedOrSilenced(myHero, npc)
...
function FAIO_Lite_utility_functions.TargetGotDisableModifier(myHero, npc)
...
function FAIO_Lite_utility_functions.heroCanCastSpells(myHero, enemy)
...
function FAIO_Lite_utility_functions.heroCanCastItems(myHero)
...
function FAIO_Lite_utility_functions.isHeroChannelling(myHero)
...
function FAIO_Lite_utility_functions.shouldCastBKB(myHero)
	...
	local enemyTable = {}
	local enemiesAround = Entity.GetHeroesInRadius(myHero, Menu.GetValue(FAIO_Lite_options.optionDefensiveItemsBKBRadius), Enum.TeamType.TEAM_ENEMY)
	...
	if enemyTable == nil or next(enemyTable) == nil then return false end

	local tempTable = {}
	for i = 1, #FAIO_Lite_data.preemptiveBKBtable do
		if Menu.IsEnabled(FAIO_Lite.preemptiveBKB[i]) then
			table.insert(tempTable, FAIO_Lite_data.preemptiveBKBtable[i])
		end
	end

	if tempTable == nil or next(tempTable) == nil then return false end
	...
	if searchAbility ~= nil and #enemyTable >= Menu.GetValue(FAIO_Lite_options.optionDefensiveItemsBKBEnemies) then
		return true
	end
...
function FAIO_Lite_utility_functions.shouldCastSatanic(myHero, enemy)
...
return FAIO_Lite_utility_functions