...
function FAIO_Lite_skillHandler.skillIsReady(skill)
	...
	if Ability.IsHidden(skill) then
		return false
	end
...
function FAIO_Lite_skillHandler.skillIsCastable(skill, castRange, target, position, linkens)
	...
	if FAIO_Lite_utility_functions.heroCanCastItems(myHero) == false then
		return false
	end
	...
	if FAIO_Lite_utility_functions.isHeroChannelling(myHero) == true then
...
function FAIO_Lite_skillHandler.executeSkillOrder(skill, target, position)
	...
	if Ability.GetBehavior(skill) & Enum.AbilityBehavior.DOTA_ABILITY_BEHAVIOR_NO_TARGET ~= 0 then
		...
		FAIO_Lite_skillHandler.mainTick = os.clock() + 0.055 + NetChannel.GetAvgLatency(Enum.Flow.FLOW_OUTGOING)
		return
	...
	elseif Ability.GetBehavior(skill) & Enum.AbilityBehavior.DOTA_ABILITY_BEHAVIOR_UNIT_TARGET ~= 0 and Ability.GetBehavior(skill) & Enum.AbilityBehavior.DOTA_ABILITY_BEHAVIOR_POINT == 0 then
		...
		FAIO_Lite_skillHandler.mainTick = os.clock() + 0.055 + NetChannel.GetAvgLatency(Enum.Flow.FLOW_OUTGOING) + turnAdjustment
		return
	...
	elseif Ability.GetBehavior(skill) & Enum.AbilityBehavior.DOTA_ABILITY_BEHAVIOR_POINT ~= 0 then
		...
		local mousePos = Input.GetWorldCursorPos()
		local turnAdjustment = FAIO_Lite_utility_functions.TimeToFacePosition(myHero, pos)
